...
async def admin_adding_menu_kb():
    inline_keyboard = [
        ...
        [
            InlineKeyboardButton(text=amsg.ADMIN_BUTTON_ADD_GROUP, callback_data=amsg.ADMIN_BUTTON_ADD_GROUP)
        ],
        [
...
async def admin_adding_task_menu_kb():
    inline_keyboard = [
        ...
        [
            InlineKeyboardButton(text=amsg.ADMIN_BUTTON_ADD_TASK_BY_SHEMA, callback_data=amsg.ADMIN_BUTTON_ADD_TASK_BY_SHEMA)
        ],
        [
...
async def admin_adding_task_by_schema_kb(adding_word_list : list = None,
                                         adding_user_list : list = None,
                                         adding_study_day : bool = False,
                                         confirmation : bool = False):
    builder = InlineKeyboardBuilder()
    ...
    if adding_word_list:
        for word in adding_word_list:
            builder.button(text=f'{word}', callback_data=f'{callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_WORD}{word[:15]}')
        builder.button(text=amsg.ADM_CONFIRM,
                       callback_data=f'{callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_WORD}{callmsg.CALL_ADMIN_END_CHOOSING}')

    if adding_user_list:
        for user in adding_user_list:
            builder.button(text=f'{user}', callback_data=f'{callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_USER}{user[:15]}')
        builder.button(text=amsg.ADM_CONFIRM,
                       callback_data=f'{callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_USER}{callmsg.CALL_ADMIN_END_CHOOSING}')

    if adding_study_day:
        for i in range(12):
            day = (date.today() + timedelta(days=i)).strftime("%d.%m.%Y")
            builder.button(text=f'-{day}-', callback_data=f'{callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_DAY}{day}')


    if confirmation:
        builder.button(text=amsg.ADMIN_BUTTON_CONFIRM_DATA,
                       callback_data=callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_CONF + cmsg.YES)
        builder.button(text=amsg.ADMIN_BUTTON_NO_CONFIRM_DATA,
                       callback_data=callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_CONF + cmsg.NO)
        builder.button(text='in devops', callback_data='no call dev')
...
# inline keyboard adding group
async def admin_adding_group_kb(adding_user_list : list = None,
                                adding_level : bool = False,
                                confirmation : bool = False):
    builder = InlineKeyboardBuilder()
    ...
    if adding_user_list:
        for user in adding_user_list:
            builder.button(text=f'{user}', callback_data=f'{callmsg.CALL_ADM_ADD_GROUP_USER}{user[:15]}')
        builder.button(text=amsg.ADM_CONFIRM,
                       callback_data=f'{callmsg.CALL_ADM_ADD_GROUP_USER}{callmsg.CALL_ADMIN_END_CHOOSING}')

    if adding_level:
        for level in cmsg.COMMON_LIST_STUDY_LEVELS:
            builder.button(text=f'-{level}-', callback_data=f'{callmsg.CALL_ADM_ADD_GROUP_LEVEL}{level}')

    if confirmation:
        builder.button(text=amsg.ADMIN_BUTTON_CONFIRM_DATA,
                       callback_data=callmsg.CALL_ADM_ADD_GROUP_CONF + cmsg.YES)
        builder.button(text=amsg.ADMIN_BUTTON_NO_CONFIRM_DATA,
                       callback_data=callmsg.CALL_ADM_ADD_GROUP_CONF + cmsg.NO)
