...
async def admin_adding_homework_start(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(user_id_list=[])
...
async def admin_adding_homework_capture_hometask(message: Message, state: FSMContext):
    ...
    group_list = await aut.get_groups_list_for_kb_with_limit()
    kb_list=group_list+user_list
    await state.update_data(users_kb=user_list)
    reply_kb = await akb.admin_adding_homework_kb(adding_user_list=kb_list)
...
async def admin_adding_homework_capture_users_from_call(call: CallbackQuery, state: FSMContext):
    ...
    user_id_list = (await state.get_data()).get('user_id_list')
    ...
    if user_call == callmsg.CALL_ADMIN_END_CHOOSING and have_check:
        ...
        if user_call.startswith('Group:'):
            group_call = user_call.replace('Group:','')
            group_id = int(group_call.split('-', 1)[0])
            user_string_from_group = (await rq.get_groups_by_filters(group_id=group_id))[0].users
            user_list_from_group = user_string_from_group.split(",")
            user_list_from_group = [int(x) for x in user_list_from_group]
            user_id_list.extend(user_list_from_group)
        else:
            user_id = int(user_call.split('-', 1)[0])
            user_id_list.append(user_id)
        await state.update_data(user_id_list=user_id_list)
        test = await aut.set_check_in_id_list_for_kb(checked_list_starts_with_id=users_list, checked_items=user_id_list)
