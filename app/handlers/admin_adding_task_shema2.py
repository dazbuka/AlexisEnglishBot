...
admin_adding_task_by_shema_router = Router()

class AddTaskBySchema(StatesGroup):
    author = State()
    words_kb = State()
    medias_kb = State()
    users_kb = State()
...
@admin_adding_task_by_shema_router.callback_query(F.data == amsg.ADMIN_BUTTON_ADD_TASK_BY_SHEMA)
async def admin_adding_task_start(call: CallbackQuery, state: FSMContext):
    await state.clear()
    # получаем список слов или медиа
    word_list = await aut.get_word_list_for_kb_with_ids_limited()
    reply_kb = await akb.admin_adding_task_by_schema_kb(adding_word_list=word_list)
    message_text = amsg.ADM_ADD_TASK_BY_SCHEMA_WORD
    await state.update_data(words_kb=word_list)
    await state.set_state(AddTaskBySchema.words_kb)
...
# хендлер захвата слова из клавиатуры копок!
@admin_adding_task_by_shema_router.callback_query(F.data.startswith(callmsg.CALL_ADM_ADD_TASK_BY_SCHEMA_WORD),
                                                  AddTaskBySchema.words_kb)
async def set_task_by_schema_capture_words_from_call(call: CallbackQuery, state: FSMContext):
